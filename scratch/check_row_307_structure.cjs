...
async function run() {
  ...
  // Let's search if there's any row containing "임대인" or "월세" to verify where the monthly rent table starts
  for (let r = 250; r <= 332; r++) {
    ...
    for (let c = 1; c <= 38; c++) {
      ...
      if (text.includes('임대인') || text.includes('월세액') || text.includes('임대차')) {
        console.log(`Found keyword at Row ${r}, Col ${excelColumnName(c)}: "${text}"`);
      }
    }
  }
}
