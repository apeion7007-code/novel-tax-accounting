...
export const CustomerConsultationForm: React.FC<CustomerConsultationFormProps> = ({
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '20px', marginTop: '20px' }}>
            ...
            <div style={{
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (regForm.clientId) {
                    updateClientFeeRate(regForm.clientId, rate, `후불 ${rate}%`).then(res => {
                      if (res.success) {
                        showToast(`수수료율이 ${rate}%로 슈퍼베이스에 실시간 저장되었습니다.`, 'success');
                      }
                    });
                  }
                }}
      ...
      {/* 📄 서명된 계약서 전체 보기 및 인쇄 모달 (A4 2페이지 표준 규격) */}
      {showFullContractModal && regForm.contractSignatureUrl && (() => {
        const stored = getStoredContractTranslations();
        const currentTranslation = stored[selectedContractLang] || stored['한국어'] || CONTRACT_TRANSLATIONS[selectedContractLang] || CONTRACT_TRANSLATIONS['한국어'];
        
        // Fee Calculations from DB / regForm (Preserving exact signed feeRate)
        const feeMethod = regForm.feePaymentStatus || '후불 22%';
        const numericFeeRate = Number(regForm.feeRate) || (regForm.feePaymentStatus ? Number(regForm.feePaymentStatus.match(/\d+/)?.[0] || 0) : 0) || selectedFeeRate || 22;
        ...
        const totalFeeRate = (prepaidRate > 0 || postpaidRate > 0) ? (prepaidRate + postpaidRate) : numericFeeRate;

        // Calculate expected total refund across active years
        let totalRefund = 0;
        const years = regForm.years || [];
        const targetYears = Array.from(new Set(years.filter((y: any) => y.active).map((y: any) => String(y.year))));
        targetYears.forEach((yr: any) => {
          const res = calculateCombinedRefund(regForm, yr, totalFeeRate);
          if (res && res.finalRefund > 0) {
            totalRefund += res.finalRefund;
          }
        });

        const contractData: ContractData = {
          name: regForm.name || '',
          country: regForm.nationality || '',
          regNum: regForm.foreignerNumber || regForm.cleanRegNum || regForm.birthDate || '',
          company: regForm.companyName || '-',
          visa: regForm.visa || '-',
          totalRefund: totalRefund,
          feeRate: totalFeeRate,
          prepaidRate: prepaidRate,
          postpaidRate: postpaidRate,
          signatureUrl: regForm.contractSignatureUrl,
          signedDate: regForm.contractConsentDate ? new Date(regForm.contractConsentDate).toLocaleDateString('ko-KR', { year: 'numeric', month: 'long', day: 'numeric' }) : new Date().toLocaleDateString('ko-KR')
        };
        ...
        return (
          ...
          >
            ...
            <div 
              ...
              onClick={(e) => e.stopPropagation()}
            >
              <div style={{ color: '#ffffff', fontWeight: 'bold', fontSize: '13px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                <span>📄 서명 완료된 세무 경정청구 표준계약서</span>
                <span style={{ fontSize: '11px', backgroundColor: '#10b981', color: '#ffffff', padding: '2px 8px', borderRadius: '4px' }}>
                  약정 수수료 {totalFeeRate}%
                </span>
              </div>
              <div style={{ display: 'flex', gap: '8px' }}>
                <button
                  type="button"
                  onClick={() => printA4ContractDocument('#printable-contract-modal-overlay')}
                  style={{
                    padding: '6px 14px',
                    fontSize: '12px',
                    backgroundColor: '#0284c7',
            ...
            >
              <A4ContractDocument
                language={selectedContractLang}
                translations={currentTranslation}
                onLanguageChange={setSelectedContractLang}
